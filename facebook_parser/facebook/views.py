                ...
                for key, value in words_dict.get(participant, "").items():
                    for word in Counter(value).most_common(3):
                        if key == "four":
                            ...
                            this_word = TenAndMoreCharWord(
                                participant=participant_model, count=word[1], word=[0]
                            )
                            this_word.save()
            end = process_time()
...
class ChartView(TemplateView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        for user in ParticipantModel.objects.all():
            ...
            reactions_queryset = ParticipantModel.objects.filter(name=user.name).values(
                "most_common_reaction"
            )
            reaction_list.append(reactions_queryset[0].get("most_common_reaction"))
            number_of_reactions_queryset = ParticipantModel.objects.filter(
                name=user.name
            ).values("counter_most_common_react")
            numbers_react_list.append(
                number_of_reactions_queryset[0].get("counter_most_common_react")
            )
            four_char_words = list(
                FourCharWord.objects.filter(participant=user).values("count", "word")
            )
            five_char_words = list(
                FiveCharWord.objects.filter(participant=user).values("count", "word")
            )
            six_char_words = list(
                SixCharWord.objects.filter(participant=user).values("count", "word")
            )
            seven_char_words = list(
                SevenCharWord.objects.filter(participant=user).values("count", "word")
            )
            eight_char_words = list(
                EightCharWord.objects.filter(participant=user).values("count", "word")
            )
            nine_char_words = list(
                NineCharWord.objects.filter(participant=user).values("count", "word")
            )
            ten_char_words = list(
                TenAndMoreCharWord.objects.filter(participant=user).values(
                    "count", "word"
                )
            )
        ...
        context.update(
            reaction_list=sorted(
                zip(users_list, reaction_list, numbers_react_list),
                key=lambda x: x[2],
                reverse=True,
            )
        )
        ...
        context.update(
            user_messages_list=sorted(
                zip(users_list, messages_list), key=lambda x: x[1], reverse=True
            )
        )
        context.update(
            user_characters_list=sorted(
                zip(users_list, characters_list), key=lambda x: x[1], reverse=True
            )
        )
        context.update(
            user_words_list=sorted(
                zip(users_list, words_list), key=lambda x: x[1], reverse=True
            )
        )
        context.update(
            user_photos_list=sorted(
                zip(users_list, photos_list), key=lambda x: x[1], reverse=True
            )
        )
        context.update(
            user_links_list=sorted(
                zip(users_list, links_list), key=lambda x: x[1], reverse=True
            )
        )
        context.update(
            user_gifs_list=sorted(
                zip(users_list, gifs_list), key=lambda x: x[1], reverse=True
            )
        )
