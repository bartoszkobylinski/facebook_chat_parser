...
class FacebookChat:
    ...
    def find_chat_word_frequency(self, word: str):
        ...
        for message in self.messages:
            messages_string = (
                messages_string
                + self.correct_string_decoding(message.get("content", " ")).lower()
                + " "
            )
        ...
        for message in self.messages.get("messages", ""):
            if message.get("reactions", ""):
                for reaction in message.get("reactions", ""):
                    reactions.extend(
                        tuple(reaction.keys(), reaction.get(reaction.keys(), ""))
                    )
    ...
    def determine_chat_word_frequency_by_word_length(self, word_len: int):
        messages = []
        for message in self.messages:
            message = self.correct_string_decoding(message.get("content", " ")).lower()
    ...
    def creating_dict_with_words_occurance_sorted_by_length(self):
        ...
        for message in self.messages.get("messages", ""):
            if (
                self.correct_string_decoding(message.get("sender_name", ""))
                in words_dict
            ):
    ...
    def get_fb_chat_words(self):
        ...
        for content in self.messages.get("messages", ""):
            if content.get("content", ""):
                temp_list = (
                    self.correct_string_decoding(content["content"]).lower().split(" ")
                )
